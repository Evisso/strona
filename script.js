...
function inicjalizujWykladowcow() {
    ...
    // Usuń przypisanych wykładowców z listy wykladowcy
    resztaWykladowcow = zapisaniWykladowcy
        ? zapisaniWykladowcy.filter(wykl => !przypisaniWykladowcy.includes(wykl))
        : wykladowcy.slice(4).filter(wykl => !przypisaniWykladowcy.includes(wykl));
}
...
function wyswietlWykladowcow() {
    ...
    if (zapisaniWidoczni && zapisaniWidoczni.length > 0) {
        ...
        widoczni.push(...wykladowcy.slice(0, 4).filter(wykl => !przypisaniWykladowcy.includes(wykl)));
    }
...
function dodajOsobeDoKomorki(komorka) {
    if (wybranaOsoba) {
        ...
        // Dodaj wybraną osobę do listy przypisanych wykładowców
        const przypisaniWykladowcy = JSON.parse(localStorage.getItem('przypisaniWykladowcy')) || [];
        przypisaniWykladowcy.push(wybranaOsoba);
